...
static char _acBuffer[1024 * 4]={0x1};
...
/**
  ...
  */
int _GetData(void * p, const U8 ** ppData, unsigned int NumBytesReq, U32 Off)
{
	...
	/* 进入临界段 */
//	taskENTER_CRITICAL();
	result = f_read(&file, _acBuffer, (int)NumBytesReq, &NumBytesRead);
	/* 退出临界段 */
//	taskEXIT_CRITICAL();
...
{
  /* 等待信号量 */
	xSemaphoreTake(FatFs_Ready_Handle,/* 二值信号量句柄 */
								 portMAX_DELAY);/* 阻塞等待 */  
	/* 设置背景色 */
