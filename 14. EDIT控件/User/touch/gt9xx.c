...
const uint8_t CTP_CFG_GT5688[] =  {
	0x97,0xE0,0x01,0x56,0x03,0x05,0x35,0x00,0x01,0x00,
    ...
    if (touch_num)
    {
        for (i = 0; i < touch_num; i++)						//一个点一个点处理
        {
            ...
            input_w  = coor_data[5] | (coor_data[6] << 8);	//size
						input_x  = 480 - input_x;//X左边是反的,这里处理一下
            {
 ...
 int32_t GTP_Init_Panel(void)
{
    ...
    memcpy(&config[GTP_ADDR_LENGTH], cfg_info, cfg_info_len);
		...
		/*根据LCD的扫描方向设置分辨率*/
		config[GTP_ADDR_LENGTH+1] = 480 & 0xFF;
		config[GTP_ADDR_LENGTH+2] = 480 >> 8;
		config[GTP_ADDR_LENGTH+3] = 854 & 0xFF;
		config[GTP_ADDR_LENGTH+4] = 854 >> 8;
